...
arg_parser.add_argument("-s", "--kernelshape", default="ellipse", help="Shape of the kernel for dilation",
...
def background_subtractor(img, dilsize=5, blursize=59, kernelshape="ellipse", bg_norm=False, bg_norm_min=20,
                          bg_norm_max=235, bluriter=1, fix_blursize=False, textdilation=True, contrast=False,
                          verbose=False):
    ...
    for idx, plane in enumerate(rgb_planes):
        ...
        if bg_norm:
            bg_img = cv2.normalize(bg_img, None, alpha=bg_norm_min, beta=bg_norm_max, norm_type=cv2.NORM_MINMAX,
                                   dtype=cv2.CV_8U)
...
def main():
    ...
    for img in args.fname:
        ...
        bg_sub = background_subtractor(img, dilsize=args.dilsize, blursize=args.blursize, kernelshape=args.kernelshape,
                                       bg_norm=args.bg_normalize, bg_norm_min=args.bg_normalize_min,
                                       bg_norm_max=args.bg_normalize_max,
